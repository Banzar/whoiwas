...
class MemoriesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create memory" do
    assert_difference('Memory.count') do
      post memories_url, params: { memory: { legacy_id: @memory.legacy_id, title: @memory.title, what_happened: @memory.what_happened } }
    end
  ...
  test "should update memory" do
    patch memory_url(@memory), params: { memory: { legacy_id: @memory.legacy_id, title: @memory.title, what_happened: @memory.what_happened } }
